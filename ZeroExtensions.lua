...
function ZeroExtensions.RequestSearchManga(query)
	...
	local request = ZeroExtensions.GetRequest(string.format("http://www.zerobywns.com/plugin.php?id=jameson_manhua&a=search&c=index&keyword=女子高生&page=%s",query,1));
	request.Callback=callBack;
	request:Send();
end
